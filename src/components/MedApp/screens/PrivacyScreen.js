...
export const PrivacyScreen = ({ setCurrentPage }) => (
  <div style={{ ...style.screen, background: "#fff" }}>
    <PageHeader title="Privacy Policy" onBack={() => setCurrentPage("home")} />
    <div style={{ ...layout.narrowContent, padding: "14px 0 40px" }}>
      {privacySections.map((section) => (
        <div key={section.heading} style={{ marginBottom: 20 }}>
          <p style={{ color: theme.green, fontWeight: 800, fontSize: 14, marginBottom: 6, marginTop: 0 }}>{section.heading}</p>
          <p style={{ fontSize: 13, color: "#111", lineHeight: 1.78, textAlign: "justify", margin: 0, whiteSpace: "pre-line" }}>{section.body}</p>
        </div>
      ))}
    </div>
  </div>
